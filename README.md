...
Отслеживаемый корневой `.env` служит безопасным шаблоном: несекретные значения
сохранены, а поля credentials пустые. Compose на целевом сервере получает
рабочую копию явно через `--env-file /etc/music-verifier.env`; администратор
заполняет только её и устанавливает mode 600. Рабочий файл нельзя коммитить;
значения не помещаются в Dockerfile, image или Compose YAML.
...
Корневой `.env` сохранён в Git как безопасный шаблон с несекретными настройками
и пустыми полями секретов. На целевом сервере скопируйте его в защищённый
`/etc/music-verifier.env`, заполните только рабочую копию и установите mode 600;
отслеживаемый `.env` не редактируйте. `data/`, tests, offline reference data,
venv, node_modules, Git и временные artifacts исключены из build context/final
images.
