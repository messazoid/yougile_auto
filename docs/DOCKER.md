...
At browser startup, `cisnet-playwright/configure-profile.js` updates only the
`Default/Preferences` settings for password saving, autofill, translation, and
site permission prompts. It keeps unrelated profile settings, cookies, and the
login session. The file is written atomically before Chromium starts; invalid
profile JSON stops startup instead of replacing the existing profile. These
settings affect Chromium's own prompts, not dialogs generated by CIS-Net.
